...
route.get('/api/zjgl/m1/sbcj/getdata', async (ctx, next) => {
    await tl.lock.acquire('dataCache',async function(done) {
        try {
            let params=ctx.query
            let logininfo = token.getinfoFromReq(ctx)
            let where = "where bklb='1' and xh not in " + SysCache.dataCache.get("m1", "sbcj")
            let start = 1
            ...
            ret.data =result
            ret["pagecacheid"] = SysCache.dataCache.add(logininfo.appid, "m1", "sbcj", logininfo.ID, xhs)   
...
route.post('/api/zjgl/m1/sbcj/shtg', async (ctx, next) => {
    try {
...
route.post('/api/zjgl/m1/sbcj/sqshbtg', async (ctx, next) => {
    try {
        let params=ctx.request.body
        let logininfo = token.getinfoFromReq(ctx)
        //var ip =ctx.headers['x-real-ip'] || ctx.headers['x-forwarded-for'] || ctx.connection.remoteAddres || ctx.socket.remoteAddress || '';
