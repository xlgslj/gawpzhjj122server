    ...
    return new Promise(async (resolve, reject) => {
        try {
            let maxsj = (await sysconfg.getrows('sjclsj', (item) => {
                return item.MK === '民警录入' && item.SUBSYS === '非现场执法'
            }))[0].SJ
    ...
    return new Promise(async (resolve, reject) => {
        try {
            let v = {
                XH: params.data.XH,
                HPHM : params.hphm,
                HPZLNO : params.hpzlno,
                HPNAME : params.hpname,
                JDCSYR : params.jdcsyr,
                XXDZ : params.xxdz,
                DH : params.dh,
                CGCLPP : params.cgclpp,
                CGCSYS : params.cgcsys,
                SYXZ : params.syxz,
                SYXZ2 : params.syxz2,
                CLLX : params.cllx,
                CLLX2 : params.cllx2,
                WZXWNO : params.wzxwno,
                WZXW : params.wzxw,
                HDBJ : 'Y'
            }
...
const imgHc = (params) => {
    return new Promise((resolve, reject) => {
        let worker = childProcess.fork('./src/task/ddcs_imghc.ts', [JSON.stringify(params)])
