...
import * as mjlrsrv from './mjlrsrv'

let route = new CRobj();

route.get('/api/zjgl/m3/mjlr/getdata', async (ctx, next) => {
    await tl.lock.acquire('dataCache',async function(done) {
        try {
            let params=ctx.query
            let logininfo = token.getinfoFromReq(ctx)
            let where = "where bklb='3' and hdbj='S' and xh not in " + SysCache.dataCache.get("m3", "mjlr")
            let start = 1
            ...
            ret.data =result
            ret["pagecacheid"] = SysCache.dataCache.add(logininfo.appid, "m3", "mjlr", logininfo.ID, xhs)   
...
route.post('/api/zjgl/m3/mjlr/shtg', async (ctx, next) => {
    try {
        ...
        await mjlrsrv.shtghandler(params)
...
route.post('/api/zjgl/m3/mjlr/del', async (ctx, next) => {
    try {
        let params=ctx.request.body
        let logininfo = token.getinfoFromReq(ctx)
        //var ip =ctx.headers['x-real-ip'] || ctx.headers['x-forwarded-for'] || ctx.connection.remoteAddres || ctx.socket.remoteAddress || '';
        ...
        params.loginname = logininfo.loginname
        params.name = logininfo.NAME
        await mjlrsrv.delhandler(params)
        let ret = new  IChttp.CRet(1);
        ret['hdbj'] = '5'
        ctx.response.body = ret;
    } catch (e) {
